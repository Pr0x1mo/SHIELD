...
class SmartReportGUI:
    ...
    def setup_ui(self):
        ...
        tk.Button(bottom, text="Load Report File", command=self.load_file).pack(anchor="w")
        self.preview = scrolledtext.ScrolledText(bottom, height=20, font=("Courier New", 10))
        self.preview.pack(fill="both", expand=True)

        control_frame = tk.Frame(bottom)
        control_frame.pack(fill="x", pady=3)

        tk.Label(control_frame, text="Highlight Group:").pack(side="left")
        self.group_highlight_var = tk.StringVar(value="All")
        self.group_highlight_menu = ttk.Combobox(control_frame, textvariable=self.group_highlight_var, state="readonly")
        self.group_highlight_menu.pack(side="left", padx=5)
        self.group_highlight_menu["values"] = ["All"] + [str(i) for i in range(1, 11)]

        tk.Button(control_frame, text="Apply Group Highlighting", command=self.render_preview).pack(side="left", padx=5)
    ...
    def render_preview(self):
        ...
        lines = self.preview_lines[header_skip:len(self.preview_lines) - footer_skip][:100]
